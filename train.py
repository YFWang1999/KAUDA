...
def train():
    ...
    for step in range(conf.train.min_step + 1):
        ...
        spurity_thr = get_thre(step,ratio,tao)
        #purity_thr = 0.1 + conf.train.ttt
        #purity_thr = 0.
        if step % 600 == 0 and step>99:
            print('begin cluster')
            feature_list,s_index=get_list(inputs,G)
            cluster_id = KMeans(feature_list,k=4000,pca_dim=256)
            
            source_loader.dataset.cluster[:]=-1
            target_loader.dataset.cluster[:]=-1
            source_loader.dataset.cluster=cluster_id[0:len(s_index)]
            target_loader.dataset.cluster=cluster_id[len(s_index):]
            
            cluster_s_all=source_loader.dataset.cluster
            cluster_t_all=target_loader.dataset.cluster
            cluster_all=np.concatenate((cluster_s_all,cluster_t_all),axis=0)
            unq_cls = np.unique(cluster_id)

            pure=[-1 for iii in range(len(unq_cls))]
            for k in range(len(unq_cls)):
                sx=np.nonzero(cluster_s_all==unq_cls[k])[0]
                ax=np.nonzero(cluster_all==unq_cls[k])[0]
                pure[k]=len(sx)/len(ax)
        ...
        if not args.no_adapt:
            ...
            coef = np.zeros(len(data_t[3]))
            ind=[]
            cluster = data_t[3]
            unq_cluster = np.unique(cluster)
            for i in range(len(unq_cluster)):
                if unq_cluster[i]==-1:
                    pass
                elif pure[unq_cluster[i]]<=purity_thr :
                    ind=np.nonzero(cluster==unq_cluster[i])[0]
                    coef[ind]=1
            ent_open = open_entropy(out_open_t,coef)
